 ...
 */
function newscenter_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'newscenter_form_submit';
}


function newscenter_form_submit($form, &$form_state) {
  // Import database newscenter file.
  $newscenter_file = dirname(__FILE__) . '/newscenter.sql';
  $success = import_newscenter($newscenter_file);
...
/**
 * Imports a database newscenter file.
 *
 * @see demo_reset().
 */
function import_newscenter($filename) {

  global $databases;
  $db_prefix = $databases["default"]["default"]["prefix"];
  
  // Open newscenter file.
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open newscenter file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
  ...
  // Load data from newscenter file.
  $success = TRUE;
  $query = '';
  $new_line = TRUE;
